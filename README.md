...
keystone-sim-grid-stack is a C# project for simulations. It focuses on this technical goal: Create a C# reference implementation for grid workflows, centered on incremental indexing, append-only fixtures, and checkpoint recovery checks.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 163, risk penalty 5, latency penalty 2, and weight bonus 4. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
