...
`keystone-sim-grid-stack` is centered on simulations work in C#. The technical target is: Create a C# reference implementation for grid workflows, centered on incremental indexing, append-only fixtures, and checkpoint recovery checks.
...
The score is deterministic. The current threshold is 163. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 4.
